...
export default function SuperAdminDashboard() {
  ...
  return (
    <div className="min-h-screen lg:h-screen bg-[#edf1f5] flex flex-col lg:flex-row font-sans antialiased text-slate-800 lg:overflow-hidden">
      ...
      {/* Mobile Top Bar */}
      <header className="lg:hidden bg-[#0a0d10] border-b border-slate-900 px-4 py-3 flex items-center justify-between z-30 sticky top-0 text-white">
        ...
        <div className="flex items-center gap-2">
          <div className="bg-slate-900 border border-slate-800 px-3 py-1.5 rounded-xl text-[11px] font-semibold text-slate-300 flex items-center gap-1.5">
            <span className="text-slate-500">Access:</span>
            <span className="text-[#e15b3e]">Root</span>
          </div>
          <button
            onClick={() => setMobileSidebarOpen(!mobileSidebarOpen)}
            className="p-2 rounded-xl text-slate-400 hover:bg-slate-900 hover:text-white transition-colors focus:outline-none"
      ...
      {mobileSidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black/60 backdrop-blur-xs flex animate-in fade-in duration-200">
          <div className="w-72 bg-[#0a0d10] text-slate-400 h-full p-6 flex flex-col justify-between shadow-2xl animate-in slide-in-from-left duration-200 border-r border-slate-900">
      ...
      <aside className="hidden lg:flex w-64 bg-[#0a0d10] border-r border-slate-900 flex-col justify-between p-6 shrink-0 text-slate-400 z-30 h-full">
